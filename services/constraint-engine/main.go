...
// toolkitForUser fetches saved snippets from the learn-engine (best-effort).
...
// suggestToolkit surfaces the user's saved snippets relevant to this level.
func suggestToolkit(a *Archetype, level int, userID string) []map[string]any {
	...
	for _, item := range tk {
		...
		case strings.Contains(a.Slug, "rate") && strings.Contains(key, "token"):
			relevant = true
		case strings.Contains(key, "cache"): // caching snippet is broadly useful
			relevant = true
		}
...
func computeScore(s *Session) int {
	// Levels 50% / hints 15% penalty / perf 20% (lower p99 better). Max ~500.
...
// router dispatches /constraint/{id}/{action}
